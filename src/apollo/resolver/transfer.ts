...
import { IBoxTransferRequest, IBoxTransferResponse, IMigrationRequest, ITransferDetail } from '../typedef';

export const resolverTransfer = {
  Query: {
    transfers: async (
      _: any,
      { status, receiver, network, pagination }: IBoxTransferRequest,
    ): Promise<IBoxTransferResponse> => {
    ...
    migrations: async (
      _: any,
      { sender, receiver, network, pagination }: IMigrationRequest,
    ): Promise<IBoxTransferResponse> => {
      const imNftTransfer = new ModelTransfer(network);
